# Adapted from https://github.com/allenai/allennlp/blob/7e08298e7d17e8037b2b325bec21733ce1e2fd20/allennlp/data/token_indexers/wordpiece_indexer.py
...
class WordpieceIndexerModified(TokenIndexer[int]):
    """
    ...
    def tokens_to_indices(self,
        ...
        for i, token in enumerate(text):
            if token.lower() == self.separator_token.lower():
                break     
        first_separator_index = i

        question_wordpiece_ids = [[self.vocab[wordpiece] for wordpiece in self.wordpiece_tokenizer(token)]
                                 for token in text[first_separator_index:][:self.max_question_tokens]] 
        # Includes [SEP]
        question_flat_wordpiece_ids = [wordpiece for token in question_wordpiece_ids for wordpiece in token]
        # self._end_piece_ids_eff are effective wordpieces we add at the end.
        self._end_piece_ids_eff = question_flat_wordpiece_ids + self._end_piece_ids

        # Everything from now, corresponds to passage tokens. We use sliding window approach on
        # the passage tokens, while question tokens are fixed and added to the end. 
        ...
        token_wordpiece_ids = [[self.vocab[wordpiece] for wordpiece in self.wordpiece_tokenizer(token)]
                               for token in text[:first_separator_index]]
        ...
        # Note the above is all zeros. We will later add type_ids for question.
        flat_history_encoding = self._get_encoding(tokens[:first_separator_index], 'pos_')
        flat_turn_encoding = self._get_encoding(tokens[:first_separator_index], 'tag_')
        flat_scenario_encoding = self._get_encoding(tokens[:first_separator_index], 'dep_')
        ...
        window_length = self.max_pieces - len(self._start_piece_ids) - len(self._end_piece_ids_eff)
        assert window_length > 8 # Assertion can fail when question is too large.
        ...
        for token in question_wordpiece_ids:
            # For initial offsets, the current value of ``offset`` is the start of
            # the current wordpiece, so add it to ``offsets`` and then increment it.
            if self.use_starting_offsets:
                offsets.append(offset)
        ...
        if len(flat_wordpiece_ids) <= window_length:
            ...
            history_encoding = self._pad(flat_history_encoding)
            turn_encoding = self._pad(flat_turn_encoding)
            scenario_encoding = self._pad(flat_scenario_encoding)
            ...
            history_encoding = self._pad(flat_history_encoding[:pieces_accumulated])
            turn_encoding = self._pad(flat_turn_encoding[:pieces_accumulated])
            scenario_encoding = self._pad(flat_scenario_encoding[:pieces_accumulated])
        else:
            ...
            history_encoding_windows = [self._pad(flat_history_encoding[i:i + window_length])
                                        for i in range(0, len(flat_history_encoding), stride)]
            turn_encoding_windows = [self._pad(flat_turn_encoding[i:i + window_length])
                                     for i in range(0, len(flat_turn_encoding), stride)]            
            scenario_encoding_windows = [self._pad(flat_scenario_encoding[i:i + window_length])
                                         for i in range(0, len(flat_scenario_encoding), stride)]
            ...
            if last_window == penultimate_window[-len(last_window):]:
                ...
                history_encoding_windows = history_encoding_windows[:-1]
                turn_encoding_windows = turn_encoding_windows[:-1]
                scenario_encoding_windows = scenario_encoding_windows[:-1]

            token_type_ids = [token_type for window in token_type_windows for token_type in window]
            history_encoding = [encoding for window in history_encoding_windows for encoding in window]
            turn_encoding = [encoding for window in turn_encoding_windows for encoding in window]
            scenario_encoding = [encoding for window in scenario_encoding_windows for encoding in window]
        ...
        return {index_name: wordpiece_ids,
                ...
                "scenario_gold_encoding": scenario_encoding}

    def _add_start_and_end(self, wordpiece_ids: List[int]) -> List[int]:
        return self._start_piece_ids + wordpiece_ids + self._end_piece_ids_eff

    def _pad(self, wordpiece_ids: List[int]) -> List[int]:
        return ([PADDING_IDX for _ in self._start_piece_ids] +
                wordpiece_ids +
                [PADDING_IDX for _ in self._end_piece_ids_eff])

    def _extend(self, token_type_ids: List[int]) -> List[int]:
        """
        Extend the token type ids by len(start_piece_ids) on the left
        and len(end_piece_ids_eff) on the right.
        """
        first = token_type_ids[0]
        last = token_type_ids[-1]
        return ([first for _ in self._start_piece_ids] +
                token_type_ids + [first] +
                [last + 1 for _ in self._end_piece_ids_eff[:-1]])

    def _get_encoding(self, tokens, attr):
        encoding = []
        for token in tokens:
            token_encoding = getattr(token, attr) if isinstance(getattr(token, attr), int) else PADDING_IDX
            token_text = token.text.lower() if self._do_lowercase and token.text not in self._never_lowercase else token.text
            encoding.extend([token_encoding for _ in self.wordpiece_tokenizer(token_text)])
        return encoding
...
@TokenIndexer.register("bert-pretrained-modified")
class PretrainedBertIndexerModified(WordpieceIndexerModified):
