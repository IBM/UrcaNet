...
class BertEmbedderModified(TokenEmbedder):
    """
    ...
    def split_indices(self, ids):
        """Splits indices into parts of length self.max_pieces"""
        split_ids = list(ids.split(self.max_pieces, dim=-1))

        # We want all sequences to be the same length, so pad the last sequence
        last_window_size = split_ids[-1].size(-1)
        padding_amount = self.max_pieces - last_window_size
        split_ids[-1] = F.pad(split_ids[-1], pad=[0, padding_amount], value=0)
...
class PretrainedBertEmbedderModified(BertEmbedderModified):
