...
from allennlp.training.metrics import Metric, BLEU, CategoricalAccuracy
...
@Model.register("bert_copynet_dual")
class BertCopyNetDualSeq2Seq(Model):
    """
    ...
    def __init__(self,
        ...
        embedding_dim = self.bert_model1._text_field_embedder.get_output_dim()
        if self.bert_model2._text_field_embedder.get_output_dim() != embedding_dim:
            raise ValueError("BertModel1 and BertModel2 must have the same hidden size.")
        self.alpha = nn.Parameter(0.5 + 0.01 * torch.randn(embedding_dim))
        self._action_predictor = Linear(embedding_dim, 4)
        self._init_decoder_projection = Linear(self.encoder_output_dim, self.decoder_output_dim)
    ...
    def forward(self,  # type: ignore
                bert_input1: Dict[str, torch.LongTensor], # for Bert
                bert_input2: Dict[str, torch.LongTensor], # for Bert
        ...
        bert_output_dict1 = self.bert_model1(bert_input1)
        pooled_output1 = bert_output_dict1['pooled_output']
        passage_representation1 = bert_output_dict1['passage_representation']

        bert_output_dict2 = self.bert_model2(bert_input2)
        pooled_output2 = bert_output_dict2['pooled_output']
        passage_representation2 = bert_output_dict2['passage_representation']

        if passage_representation1.size(1) != passage_representation2.size(1):
            raise ValueError('Check if passage in bert_input1 and bert_input2 is same.')
        
        batch_size, passage_len, hidden_size = passage_representation1.size()

        # Shape: (batch_size, hidden_size)
        pooled_output = pooled_output1 * self.alpha.unsqueeze(0).expand(batch_size, -1) +\
                        pooled_output2 * (1 - self.alpha.unsqueeze(0).expand(batch_size, -1))
        
        # Shape: (batch_size, passage_len, hidden_size)
        passage_representation = passage_representation1 * self.alpha.unsqueeze(0).unsqueeze(0).expand(batch_size, passage_len, -1) +\
                                 passage_representation2 * (1 - self.alpha.unsqueeze(0).unsqueeze(0).expand(batch_size, passage_len, -1))

        state = self._encode(source_tokens, passage_representation)
        ...
        state['passage_representation'] = passage_representation

        if target_tokens:
            state = self._init_decoder_state(state)
            mask = (label == self.vocab.get_token_index('More', namespace="labels")).float()
            answer_loss = self._forward_loss(target_tokens, target_token_ids, state, mask)
            action_loss = cross_entropy(action_logits, label)
            loss = answer_loss + 18 * action_loss
        ...
        if not self.training:
            ...
            if target_tokens:
                if self._tensor_based_metric is not None:
                    ...
                    more_index = self.vocab.get_token_index('More', namespace="labels")
                    mask = (label == more_index)
                    unmasked_examples = best_predictions[mask].size(0)
                    if unmasked_examples != 0:
                        self._tensor_based_metric(best_predictions[mask], gold_tokens[mask])  # type: ignore
                ...
                argmax_indices = numpy.argmax(action_logits.cpu().data.numpy(), axis=-1)
    ...
    def _init_decoder_state(self, state: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        init_decoder_input = final_encoder_output
    ...
    def _encode(self,
                ...
                passage_representation: torch.Tensor) -> Dict[str, torch.Tensor]:
        """
        ...
        # shape: (batch_size, max_input_sequence_length, bert_embedding_dim)
        encoder_outputs = self._dropout(passage_representation)
    ...
    def _forward_loss(self,
        ...
        loss = - (log_likelihood * mask).sum() / (mask.sum() + 1e-6)
        return loss
    ...
    def decode(self, output_dict: Dict[str, torch.Tensor]) -> Dict[str, Any]:
        ...
        output_dict["prediction"] = self._get_predicted_tokens(output_dict["predictions"],
                                                                output_dict["metadata"],
                                                                n_best=1)
        return output_dict
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        all_metrics['action_acc'] = self._action_accuracy.get_metric(reset=reset)
        if 'BLEU' in all_metrics.keys():
            agg_metric = all_metrics['BLEU'] + all_metrics['action_acc'] * (0.55)
            all_metrics['agg_metric'] = agg_metric
        return all_metrics