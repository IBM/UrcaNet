...
@Model.register("bidaf_copynet_ft")
class BiDAFCopyNetFTSeq2Seq(Model):
    """
    ...
    def __init__(self,
        ...
        if pretrained_bidaf:
            params = Params.from_file("./temp/bidaf_baseline_ft/config.json")
            vocab = Vocabulary.from_files("./temp/bidaf_baseline_ft/vocabulary")
            self.bidaf_model = Model.from_params(vocab=vocab, params=params.pop('model'))
            map_location = None if torch.cuda.is_available() else 'cpu'
            with open("./temp/bidaf_baseline_ft/best.th", 'rb') as f:
        ...
        self._action_predictor = Linear(modeling_dim, 4)
    ...
    def forward(self,  # type: ignore
                question: Dict[str, torch.LongTensor], # for BiDAF
                passage: Dict[str, torch.LongTensor], # for BiDAF        
        ...
        # Shape: (batch_size, 4)
        action_logits = self._action_predictor(modeled_passage)
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        if 'BLEU' in all_metrics.keys():
            agg_metric = all_metrics['BLEU'] + all_metrics['action_acc'] * (1/3)
