...
@Model.register("ps_pipeline")
class PSPipeline(Model):
    """
    ...
    def __init__(self,
                 ...
                 tensor_based_metric1: Metric = None,
                 tensor_based_metric2: Metric = None,
                 token_based_metric1: Metric = None,
                 token_based_metric2: Metric = None,
        ...
        self._tensor_based_metric1 = tensor_based_metric1 or \
            BLEU(exclude_indices={self._pad_index, self._end_index, self._start_index})
        self._tensor_based_metric2 = tensor_based_metric2 or \
            BLEU(exclude_indices={self._pad_index, self._end_index, self._start_index})
        
        self._token_based_metric1 = token_based_metric1
        self._token_based_metric2 = token_based_metric2
        ...
        self._input_projection_layer1 = Linear(target_embedding_dim + self.encoder_output_dim * 2, self.decoder_input_dim)
        self._input_projection_layer2 = Linear(target_embedding_dim + self.encoder_output_dim * 2, self.decoder_input_dim)

        # We then run the projected decoder input through an LSTM cell to produce
        # the next hidden state.
        self._decoder_cell1 = LSTMCell(self.decoder_input_dim, self.decoder_output_dim)
        self._decoder_cell2 = LSTMCell(self.decoder_input_dim, self.decoder_output_dim)

        # We create a "generation" score for each token in the target vocab
        # with a linear projection of the decoder hidden state.
        self._output_generation_layer1 = Linear(self.decoder_output_dim, target_vocab_size)
        self._output_generation_layer2 = Linear(self.decoder_output_dim, target_vocab_size)
        ...
        self._output_copying_layer1 = Linear(self.encoder_output_dim, self.decoder_output_dim)
        self._output_copying_layer2 = Linear(self.encoder_output_dim, self.decoder_output_dim)
    ...
    def forward(self,  # type: ignore
                ...
                target_tokens1: Dict[str, torch.LongTensor] = None,
                target_tokens2: Dict[str, torch.LongTensor] = None,
                target_token_ids1: torch.Tensor = None,
                target_token_ids2: torch.Tensor = None) -> Dict[str, torch.Tensor]:
        ...
        """
        state1 = self._encode(source_tokens)
        state1["source_token_ids"] = source_token_ids
        state1["source_to_target"] = source_to_target

        state2 = state1.copy() # shallow copy

        if target_tokens1 is not None and target_tokens2 is not None:
            state1 = self._init_decoder_state(state1)
            state2 = self._init_decoder_state(state2)

            loss1 = self._forward_loss(target_tokens1, target_token_ids1, state1, target=1)
            loss2 = self._forward_loss(target_tokens2, target_token_ids2, state2, target=2)
        ...
        if not self.training:
            ...
            output_dict['predictions1'] = self._forward_beam_search(state1, target=1)
            output_dict['predictions2'] = self._forward_beam_search(state2, target=2)
        
            labels = []
            for i in range(len(metadata)):
                labels.append(metadata[i]['label'])
            output_dict['label'] = labels
            if target_tokens1 is not None and target_tokens2 is not None:
                if self._tensor_based_metric1 is not None and self._tensor_based_metric2 is not None:
                    # shape: (batch_size, beam_size, max_sequence_length)
                    top_k_predictions1 = output_dict["predictions1"]
                    top_k_predictions2 = output_dict["predictions2"]
                    
                    # shape: (batch_size, max_predicted_sequence_length)
                    best_predictions1 = top_k_predictions1[:, 0, :]
                    best_predictions2 = top_k_predictions2[:, 0, :]

                    # shape: (batch_size, target_sequence_length)
                    gold_tokens1 = self._gather_extended_gold_tokens(target_tokens1["tokens"], source_token_ids, 
                                                                     target_token_ids1)
                    gold_tokens2 = self._gather_extended_gold_tokens(target_tokens2["tokens"], source_token_ids, 
                                                                     target_token_ids2)

                    self._tensor_based_metric1(best_predictions1, gold_tokens1)  # type: ignore
                    self._tensor_based_metric2(best_predictions2, gold_tokens2)  # type: ignore
                if self._token_based_metric1 is not None and self._token_based_metric2 is not None:
                    predicted_tokens1 = self._get_predicted_tokens(output_dict["predictions1"],
                                                                  metadata,
                                                                  n_best=1)
                    predicted_tokens2 = self._get_predicted_tokens(output_dict["predictions2"],
                                                                  metadata,
                                                                  n_best=1)

                    self._token_based_metric1(predicted_tokens1,  # type: ignore
                                             [x["target_tokens1"] for x in metadata])
                    self._token_based_metric2(predicted_tokens2,  # type: ignore
                                             [x["target_tokens2"] for x in metadata])                    
    ...
    def _decoder_step(self,
        ...
        if target == 1:
            ...
            attentive_weights = self._attention1(state["decoder_hidden"], state["encoder_outputs"], encoder_outputs_mask)
        elif target == 2:
            # shape: (group_size, max_input_sequence_length)
            attentive_weights = self._attention2(state["decoder_hidden"], state["encoder_outputs"], encoder_outputs_mask)
        ...
        if target == 1:
            ...
            projected_decoder_input = self._input_projection_layer1(decoder_input)
        elif target == 2:
            # shape: (group_size, decoder_input_dim)
            projected_decoder_input = self._input_projection_layer2(decoder_input)

        if target == 1:
            state["decoder_hidden"], state["decoder_context"] = self._decoder_cell1(projected_decoder_input,
                                                                                   (state["decoder_hidden"],
                                                                                    state["decoder_context"]))
        elif target == 2:
            state["decoder_hidden"], state["decoder_context"] = self._decoder_cell2(projected_decoder_input,
                                                                                   (state["decoder_hidden"],
                                                                                    state["decoder_context"]))
        return state

    def _get_generation_scores(self, state: Dict[str, torch.Tensor], target) -> torch.Tensor:
        if target == 1:
            return self._output_generation_layer1(state["decoder_hidden"])
        else:
            return self._output_generation_layer2(state["decoder_hidden"])

    def _get_copy_scores(self, state: Dict[str, torch.Tensor], target) -> torch.Tensor:
        ...
        if target == 1:
            ...
            copy_projection = self._output_copying_layer1(trimmed_encoder_outputs)
        elif target == 2:
            # shape: (batch_size, max_input_sequence_length - 2, decoder_output_dim)
            copy_projection = self._output_copying_layer2(trimmed_encoder_outputs)
        # shape: (batch_size, max_input_sequence_length - 2, decoder_output_dim)
    ...
    def _forward_loss(self,
        ...
        for timestep in range(num_decoding_steps):
            ...
            state = self._decoder_step(input_choices, selective_weights, state, target)
            # Get generation scores for each token in the target vocab.
            # shape: (batch_size, target_vocab_size)
            generation_scores = self._get_generation_scores(state, target)
            # Get copy scores for each token in the source sentence, excluding the start
            # and end tokens.
            # shape: (batch_size, trimmed_source_length)
            copy_scores = self._get_copy_scores(state, target)
            # shape: (batch_size,)
    ...
    def _forward_beam_search(self, state: Dict[str, torch.Tensor], target) -> Dict[str, torch.Tensor]:
        ...
        if target == 1:
            take_search_step = functools.partial(self.take_search_step, target=1)
        elif target == 2:
            take_search_step = functools.partial(self.take_search_step, target=2)
        all_top_k_predictions, log_probabilities = self._beam_search.search(start_predictions, state, take_search_step)
    ...
    def decode(self, output_dict: Dict[str, torch.Tensor]) -> Dict[str, Any]:
        ...
        the indices.
        """
        predicted_tokens1 = self._get_predicted_tokens(output_dict["predictions1"], output_dict["metadata"], n_best=1)
        predicted_tokens2 = self._get_predicted_tokens(output_dict["predictions2"], output_dict["metadata"], n_best=1)
        
        predicted_span = [metadata["predicted_span_tokens"] for metadata in output_dict["metadata"]]

        predicted_tokens = []
        for tokens1, span, tokens2 in zip(predicted_tokens1, predicted_span, predicted_tokens2):
            predicted_tokens.append(tokens1 + span + tokens2)
        output_dict['prediction'] = predicted_tokens
        return output_dict
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        if not self.training:
            if self._tensor_based_metric1 is not None and self._tensor_based_metric2 is not None:
                bleu1 = self._tensor_based_metric1.get_metric(reset=reset)['BLEU']
                bleu2 = self._tensor_based_metric2.get_metric(reset=reset)['BLEU']
                agg_bleu = bleu1 * bleu2
                all_metrics.update({'bleu1': bleu1, 'bleu2': bleu2, 'agg_bleu': agg_bleu})
            if self._token_based_metric1 is not None and self._token_based_metric2 is not None:
                all_metrics.update(self._token_based_metric1.get_metric(reset=reset))  # type: ignore
                all_metrics.update(self._token_based_metric2.get_metric(reset=reset))  # type: ignore
        return all_metrics
